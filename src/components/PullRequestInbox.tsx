...
export default function PullRequestInbox({ repositoryId }: { repositoryId: string }) {
  ...
  return <div className="pr-workspace">
    ...
    <div className="pr-layout"><div className="pr-list">{filtered.length === 0 && <div className="empty-panel"><strong>표시할 PR이 없습니다.</strong><span>OneDev에서 PR이 생성되면 이곳에 나타납니다.</span></div>}{filtered.map((item) => <button key={String(item.id ?? item.number)} className={`pr-item ${selected?.pullRequest.id === item.id ? 'selected' : ''}`} onClick={() => open(item)}><div className="pr-item-top"><span className={`status-badge status-${item.status}`}>{label(item.status)}</span><span>#{item.number ?? item.id} · {date(item.updatedAt ?? item.createdAt)}</span></div><strong>{item.title}</strong><p>{item.source ?? 'source 없음'} <b>→</b> {item.target ?? 'target 없음'}</p><small>{item.author.name}</small></button>)}</div>
      <aside className="pr-detail panel">{!selected ? <div className="detail-placeholder"><span className="index-mark">SELECT A PR</span><strong>PR을 선택하면<br />변경·리뷰·업데이트를 한곳에서 봅니다.</strong></div> : <><div className="panel-head"><div><span className={`status-badge status-${selected.pullRequest.status}`}>{label(selected.pullRequest.status)}</span><h3>#{selected.pullRequest.number ?? selected.pullRequest.id} {selected.pullRequest.title}</h3></div></div><div className="detail-body"><p className="detail-byline">{selected.pullRequest.author.name} · {selected.pullRequest.source ?? 'source 없음'} → {selected.pullRequest.target ?? 'target 없음'}</p>{selected.pullRequest.description && <p className="commit-body">{selected.pullRequest.description}</p>}<div className="pr-stat-row"><span><b>{selected.changes.length}</b> 변경</span><span><b>{selected.reviews.length}</b> 리뷰</span><span><b>{selected.comments.length}</b> 댓글</span><span><b>{selected.builds.length}</b> 빌드 정보</span></div><span className="detail-label">변경 파일</span><div className="detail-files">{selected.changes.slice(0, 40).map((change, index) => <div key={`${changePath(change)}-${index}`}><b>·</b><code>{changePath(change)}</code></div>)}</div>{selected.gitRefs && <><span className="detail-label">로컬 Git ref</span><div className="ref-pills">{(['base', 'head', 'merge'] as const).map((key) => <span key={key} className={selected.gitRefs?.[key].available ? 'ref-available' : 'ref-missing'}>{key} {selected.gitRefs?.[key].available ? selected.gitRefs?.[key].oid?.slice(0, 9) : '없음'}</span>)}</div></>}<span className="detail-label">확인 경계</span><p className="boundary-copy">PR 상태·리뷰·댓글은 OneDev 기록입니다. 테스트 통과나 업무 의도는 이 화면에서 추정하지 않습니다.</p></div></>}</aside></div>
  </div>;
}
